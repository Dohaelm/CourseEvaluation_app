  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const userCourses= courses.filter(course =>course.instructor.id==instructorId);
...
const filteredCourses = coursesWithMaxSemestre.filter(
  ...
  if (loading) return <div><h1>Dashboard</h1><p>Loading...</p></div>;
  if (error) return <div><h1>Dashboard</h1><p>{error}</p></div>;
